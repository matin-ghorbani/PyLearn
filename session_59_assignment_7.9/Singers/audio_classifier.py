    ...
    def create_dataset_for_singers(singers, dataset_path, new_dataset_path):
        ...
        for singer in tqdm(singers):
            ...
            audio = pydub.AudioSegment.from_file(
                first) + pydub.AudioSegment.from_file(second)
...
class AudioClassifier:
    ...
    def build_model(self, optimizer=Adam, loss=categorical_crossentropy):
        ...
        self.model = Sequential([
            layers.Conv1D(32, 15, 16, activation=relu,
...
def main() -> None:
    # for singer in os.listdir('./raw_data'):
    #     AudioProcessor.extract_speaker_voice(
    #         os.path.join('./raw_data', singer), './extracted_data'
    #     )

    singers = ['Ebi', 'Mohsen Chavoshi', 'Mohsen Yeganeh', 'Shayea', 'Yas']
    AudioProcessor.create_dataset_for_singers(
        singers, './extracted_data', './dataset')

    classifier = AudioClassifier('./dataset')
    classifier.make_train_validation_datasets()
    classifier.build_model()
    classifier.train_model(
        './weights/best_singer_classifier_50ep.h5', epochs=50)
    classifier.evaluate('./weights/best_singer_classifier_50ep.h5')
